...
const users = [
    { id: 1, username: 'admin', password: 'salemba41', nama: 'Administrator', role: 'Admin' },
    { id: 2, username: 'budi', password: 'operator123', nama: 'Budi', role: 'Operator' },
    { id: 3, username: 'rahayu', password: 'operator123', nama: 'Rahayu', role: 'Operator' }
];
...
function login() {
    ...
    if (!username || !password) {
        errorDiv.textContent = 'Mohon isi username dan password';
    ...
    if (user) {
        ...
        showApp();
        showToast('success', 'Selamat Datang', `Halo, ${currentUser.nama}!`);
    } else {
...
function initializeData() {
    ...
    // Sample History with user info
    history = [
        { id: 1, itemId: 1, itemName: 'Komputer Desktop HP', unitKode: 'INVT-2025-0001/004', type: 'status', date: '2025-03-01', time: '09:30', from: 'Bagus', to: 'Proses Perbaikan', description: 'Unit mengalami kerusakan pada power supply', user: 'Budi' },
        { id: 2, itemId: 2, itemName: 'Proyektor Epson', unitKode: 'INVT-2025-0002/003', type: 'status', date: '2025-02-20', time: '14:15', from: 'Bagus', to: 'Rusak', description: 'Lampu proyektor mati', user: 'Rahayu' },
        { id: 3, itemId: 4, itemName: 'Kursi Kuliah', unitKode: 'INVT-2025-0004/003', type: 'pindah', date: '2025-02-15', time: '10:00', from: 'Ruang Kelas 101', to: 'Gudang', description: 'Dipindahkan karena rusak', user: 'Administrator' },
        { id: 4, itemId: 5, itemName: 'Mikroskop Digital', unitKode: null, type: 'masuk', date: '2025-03-01', time: '08:00', from: '-', to: 'Lab Keperawatan', description: 'Hibah dari Kementerian Kesehatan (4 unit)', user: 'Rahayu' },
    ];

    // Sample Maintenance with user info
    maintenanceHistory = [
        { id: 1, assetId: 3, assetName: 'Mobil Operasional Toyota Innova', date: '2024-12-15', type: 'Service Rutin', cost: 2500000, description: 'Ganti oli dan tune up', user: 'Budi' },
        { id: 2, assetId: 4, assetName: 'Ambulans', date: '2024-11-20', type: 'Perbaikan', cost: 5000000, description: 'Perbaikan sistem AC', user: 'Administrator' },
        { id: 3, assetId: 5, assetName: 'Server Dell PowerEdge', date: '2025-01-05', type: 'Upgrade', cost: 15000000, description: 'Upgrade RAM dan storage', user: 'Administrator' }
    ];
}
...
function saveItem() {
    ...
    const newItem = {
        ...
        createdBy: currentUser ? currentUser.nama : 'Unknown',
        units: units
    };
    ...
    history.unshift({
        ...
        user: currentUser ? currentUser.nama : 'Unknown'
    });
...
function renderHistoryTimeline() {
    ...
    const icons = { 
        'masuk': '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 5v14M5 12l7 7 7-7"/></svg>',
        'pindah': '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg>',
        'status': '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20V10M18 20V4M6 20v-4"/></svg>'
    };
    document.getElementById('historyTimeline').innerHTML = history.map(h => `
        <div class="timeline-item">
            <div class="timeline-icon ${h.type}">${icons[h.type] || ''}</div>
            <div class="timeline-date">
                ${formatDate(h.date)}${h.time ? ' <span style="color:var(--text-secondary);">' + h.time + '</span>' : ''}
            </div>
            <div class="timeline-content">
                ...
                <div class="timeline-user">
                    ...
                    <span>oleh <strong>${h.user || 'Unknown'}</strong></span>
                </div>
            </div>
        </div>
    `).join('');
